...
//  Edited by Jacob Gold on 12-03-19
//  Edits include using stock NSImages and removing commented-out code
...
class ComboColorWellCell: NSActionCell {
    ...
    override func draw(withFrame cellFrame: NSRect, in controlView: NSView) {
        ...
        #imageLiteral(resourceName: "NSColorPanel").draw(in: NSInsetRect(buttonArea(withFrame: cellFrame, smoothed: true), imageInset, imageInset))
        ...
        case let .over(controlArea),
            ...
            switch controlArea {
            case .color:
                #imageLiteral(resourceName: "NSTouchBarGoDownTemplate").draw(in: popoverButtonArea(withFrame: cellFrame, smoothed: true))
            default:
                break
            }
        default:
    ...
    private func popoverButtonArea(withFrame cellFrame: NSRect, smoothed: Bool = false) -> NSRect {
        ...
        return NSRect(x: rect.width - (buttonSize + imageInset - 5),
                      y: ceil((rect.height - buttonSize) / 2),
                      width: buttonSize - 5, height: buttonSize)
    }
